...
class GameScene extends Phaser.Scene {
  ...
  _handleDAS(delta) {
    ...
    if (this._dasTimer >= (this._dasActive ? DAS_REPEAT : DAS_DELAY)) {
      this._dasTimer = 0;
  ...
  _handleFall(delta) {
    ...
    if (this._fallTimer >= speed) {
      this._fallTimer = 0;
      if (!this.board.move(this.current, 0, 1)) {
  ...
  _buildHUD() {
    ...
    this._txtCombo = this.add.text(
      BOARD_X + BOARD_W / 2, BOARD_Y + BOARD_H / 2 - 20, '',
      { fontFamily: '"Orbitron", Arial, sans-serif', fontSize: '16px', color: '#FF2D95' }
  ...
  _drawCurrent() {
    ...
    for (const [col, row] of this.current.blocks) {
      ...
      if (glowAlpha > 0) {
        g.fillStyle(this.current.color, glowAlpha * 0.25);
  ...
  _drawCombo() {
    ...
    this._txtCombo.setText(this._comboText).setAlpha(Math.min(1, this._comboTimer / 400));
  }
}
